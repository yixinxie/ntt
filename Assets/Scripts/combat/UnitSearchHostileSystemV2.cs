...
public partial class UnitSearchHostileSystemV2 : SystemBase
{
    ...
    //    }
    //}
    public static bool IsTargetInRange(float3 target_position, CachedTurretTransform c0c1, float attack_radians)
    {
        var this_dist = math.distance(target_position, c0c1.c0);

        var fwd_worldspace = math.mul(c0c1.c1, new float3(0f, 1f, 0f));
        ...
        bool ret = attack_radians < float.Epsilon || cosine > math.cos(attack_radians / 2f);
        return ret;
    }
    // todo: merge with GetTargets
    public static void GetTargets_ICD(PhysicsWorld physics, 
        ...
        in CachedTurretTransform c0c1, in CombatTeam team, Entity self_entity)//, in ColliderRef self_collider)
    {
        ...
        //for (int i = 0; i < weapons.Length; ++i)
        {
            ...
            if (physics.CalculateDistance(pdi, ref hits))
            {
                ...
                for (int j = 0; j < hits.Length// && j < CombatTargets.MaxCount
                    ; ++j)
                {
                    if (hits[j].Entity == self_entity) continue;

                    var this_dist = math.distance(hits[j].Position, c0c1.c0);
                        ...
                        (search_furthest == false && this_dist < distance_cmp))
                    {
                        ...
                        //}
                        //else
                        if (IsTargetInRange(hits[j].Position, c0c1, current_weapon.attack_radians))
                        {
    ...
    protected override void OnUpdate()
    {
        ...
        in CachedTurretTransform c0, in CombatTeam team) =>
        {
            ...
            else
            {
                ...
                if (c0_array.HasComponent(target_entity) == false || IsTargetInRange(c0_array[target_entity].Position, c0, weapons[0].attack_radians) == false)
                {
            ...
            {
                GetTargets_ICD(physics, weapons[0], out CombatTargets _targets, c0, team, entity);
                targets[0] = _targets;
            }
        }).Run();
...
public struct WeaponInfoV2 : IBufferElementData
{
    ...
    public float cooldown_left; // 0 means ready to fire, changing
    public byte burst_index; // changing
    public byte burst_max;
    public float burst_cooldown;
    public float cooldown_max;
    public float base_damage;
    public Entity addon_entity;
    public byte damamge_types;
    public uint param0;
}
