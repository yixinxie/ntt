...
Distorted_vertex warp_vertex(float3 positionOS, float4 warp_params)
{
	...
	float3 distort_origin = _WorldSpaceCameraPos + cam_forward * warp_params.z;
	...
	distorted_wspos.xyz = distorted_wspos.xyz + distort_dir * pow(distance(undistorted_cspos.xy, warp_params.xy), 2) * warp_params.w;
	//o.worldPos = wpos.xyz;
